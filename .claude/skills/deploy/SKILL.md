...
判据：**`18 page(s) built`**。页数变了先弄清为什么（本轮是否真的增删了页面），别往下走。

```bash
npx astro check
```

判据：**只剩既有的 `about.astro:19 ts(2352)`**，0 warnings。多出任何一条先修掉再提交。

上面这两个数（18 页、那一条既有错）是**会过期的判据**：本轮如果有意增删了页面或修掉了那条
既有错，报告里要说清，并**顺手把这份技能里的数字改掉** —— 判据留着旧值就等于没有判据。
