 ...
 *
 * **不要把它读成「大陆的一律打不通」。** 2026-08-25 上线后逐家实测过
 * （后台那颗「探一下能不能连」，接入机房 `colo=LAX`）：**下面十家全部可达**，
 * 401/403、0.3–2.5 秒。R37 那次的结论是「**那一家中转站**在区别对待 workerd 的请求」
 * （同一个 Worker 打 `api.openai.com` 0.5 秒就有回），而它一度被写成
 * 「打大陆机器 30 秒无响应」，在注释里躺了好几轮 —— 害得「换国内服务商」这条路一直没人试。
 *
 * 完整那张实测表在 [部署运维 · R41＋R42 部署实测](../../docs/部署运维.md)。
 * 换服务商时仍然先点那颗按钮：可达性是会变的，实测一次比信一句注释稳。
 *
 * 有 `intl` 的那几家给了**境外镜像域名**：同一家服务商的海外站。既然直连本来就通，
 * 它现在只是备用（比如哪天某家开始拦云厂商出网）。代价是海外站与国内站
 * **多数是两套账号、两把密钥**（阿里、智谱、月之暗面、硅基流动都是这样），
 * 换域名的同时要换 key。
 */
