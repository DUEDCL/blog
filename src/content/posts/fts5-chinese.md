...
给语音平台做记忆层，选的是 SQLite 加 FTS5：单文件、无服务、进程内，跟本地优先那条红线正好合。
...
ICU 分词器。FTS5 官方的答案，但 ICU 要在编译期进去，Python 自带的 `sqlite3` 没编。装扩展等于给一个单文件、无服务的方案加一个原生依赖 —— 红线是我自己定的，为了检索方便就破一次，后面每一条都可以照着破。
...
分词器的行为要自己测一遍，别信支持 Unicode 这四个字 —— `unicode61` 确实支持 Unicode，它只是不切中文。
