...
麻烦的不是这 89 行，是把它接进去要动的另外六个文件 —— adapter 注册表、服务层、agent 入口、agent 路由、前端的 API 封装和设置页。少接一处不会报错，只会在某一条调用路径上说该服务商没配置，然后你去翻另外五个文件找哪儿漏了。
...
留这一条不是为了凑数。它值一句结论：一个看起来填个 key 就能用的服务商接入，实际要穿过多少层才算真的接进去。
